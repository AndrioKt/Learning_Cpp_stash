...
//Напишем КОНТЕЙНЕРНЫЙ КЛАСС 
//вместо него зачастую лучше использовать контейнерные классы из библиотеки STL(например вектор),но для частных случаев может понадобиться собственный контейнерный класс
...
public:
	...
	//Отдельно сделаем возможность инициализации через список
	ArrayInt(const std::initializer_list<int>& list) ://передаем по ссылка, что бы избежать копирования элементов
	...
	{// произведем саму иници ализацию:
		int count = 0;
		for (auto& element : list)//цикл foreach переберает каждый элемент копируя его во внутренний массив
		{
